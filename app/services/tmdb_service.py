...
class TMDBService:
    ...
    Optimized: only fetches each collection once, not per movie.
    """
    ...
    # Step 1: Find unique collection IDs (cached, no duplicate API calls)
    seen_collections: Set[int] = set()
    collection_ids_needed: List[int] = []
    
    logger.info(f"Finding unique collections for {len(owned_tmdb_ids)} owned movies")
    ...
    for tmdb_id in owned_tmdb_ids:
        ...
        if collection_id not in seen_collections:
            ...
            collection_ids_needed.append(collection_id)
    
    logger.info(f"Found {len(collection_ids_needed)} unique collections to fetch")
    
    # Step 2: Fetch ALL collections in parallel (GAPS-2 doesn't do this, but we can!)
    if collection_ids_needed:
        tasks = [self.get_collection(cid) for cid in collection_ids_needed]
